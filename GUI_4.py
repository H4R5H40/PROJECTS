#ATTEMPT 4 TO CONVERT TERMINAL LOGIC TO GUI
import pandas as pd
import string
from tkinter import *
root=Tk()
...
def student_login():
  ...
  photo=PhotoImage(file=STUDENT_ACCOUNT)
  Label(root,text="welcome",image=photo,background="white").grid(row=0,column=1)
  Label(text="",font="Georgia 12",background="white").grid(row=4,column=1)
  Label(text="STUDENT ID::",font="Georgia 12",background="white").grid(row=1, column=0)
  Label(text="PASSWORD::",font="Georgia 12",background="white").grid(row=2,column=0)
  id=StringVar()
  password=StringVar()
  identry=Entry(root,textvariable=id,font="Georgia 12",background="white").grid(row=1,column=1)
  passentry=Entry(root,textvariable=password,show="*",font="Georgia 12",background="white").grid(row=2,column=1)
  Button(text="SUBMIT",command=check,font="Georgia 12",background="white").grid(row=3,column=1)
#====================================================================================================
def student_edit(j):
  def submit(j):
...
def print_info(i):
  ...
  global photo
  root.configure(background="white")
  photo=PhotoImage(file=STUDENR_PROFILE)
  Label(root,image=photo,background="white").grid(row=0,column=0)
  for j in range (6):
...
def admin():
  ...
  global photo
  root.configure(background="white")
  photo=PhotoImage(file=ADMIN_ACCOUNT)
  Label(root,image=photo,background="white").grid(row=0,column=1)
  Label(text="USER ID::",font="Georgia 12",background="white").grid(row=1, column=0)
  Label(text="PASSWORD::",font="Georgia 12",background="white").grid(row=2,column=0)
  id=StringVar()
  password=StringVar()
  identry=Entry(root,textvariable=id,font="Georgia 12",background="white").grid(row=1,column=1)
...
def main():
  ...
  def student():
    for widget in root.winfo_children():
      widget.destroy()
    student_login()
  Button(text="ADMIN  LOGIN / EDIT THE INFORMATION(ADMIN)",font="Georgia 12",command=admin_login,background="white").grid(row=0,column=0)
  Button(text="STUDENT LOGIN / EDIT THE INFORMATIOM(STUDENT)",font="Georgia 12",command=student,background="white").grid(row=1,column=0)
  Button(text="EXIT",font="Georgia 12",command=root.destroy,background="white").grid(row=2,column=0)
