#ATTEMPT 11 TO CONVERT TERMINAL LOGIC TO GUI
#TRAYING TO SEND REAL TIME OTP AND ALERT
import pandas as pd
import string
from tkinter import *
import random
...
def presenty():
  ...
  k=0
  l=0
  for present in range (len(df2)):
    b[present]=IntVar()
    Label(root1,text=name[present]).place(x=k,y=l*25)
    Checkbutton(root1,variable=b[present],font="Gorgia 12").place(x=k+160,y=l*25)
    '''print(b[present].get())'''
    l=l+1
    if present>=16 and present%16==0:
      k=k+200
      l=0
  Button(root1,text="SUBMIT ATTENDANCE",command=submit,font="Georgia 12",relief='solid').place(x=750,y=500)
  root1.mainloop()
#====================================================================================================
def register():
  #👇👇
