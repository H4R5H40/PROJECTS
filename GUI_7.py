...
def admin():
  def check():
    if id.get()==id_pass[0] and password.get()==id_pass[1]:
      ...
      Label(custom_box, text="INCORRECT ID PASSWORD").pack(pady=20)
      def custom_action():
        forgot_password_admin()  
        custom_box.destroy()
      def cancel_action():
        custom_box.destroy()
      Button(custom_box, text="CHANGE PASSWORD", command=custom_action).pack(side=LEFT, padx=30)
      Button(custom_box, text="RETRY", command=cancel_action).pack(side=RIGHT, padx=30)
  Label(root,image=photo).place(x=0,y=0)
  global p2
